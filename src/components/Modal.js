          ...
          <div className="modal-content flex h-screen relative justify-center items-center" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="main-content min-w-fit min-h-fit">
              ...
              <div className="modal-footer w-full flex flex-row justify-around text-white  text-3xl">
                <button className="fixed right-0 top-0 close rounded-3xl bg-slate-800 p-2" onClick={() => { onClose(false); setCounter(0); setCount(0) }}><FaWindowClose /></button>
              </div>
              <div className='w-1/2 relative flex flex-row justify-around text-white  text-3xl'>
                {count > 1 ? <button className="fixed top-2/4 left-0 rounded-3xl bg-slate-800 p-2" onClick={() => { setCounter(indexImage => indexImage - 1); }}><FaBackward /></button> : ''}
                <button className='fixed top-2/4 right-0 rounded-3xl bg-slate-800 p-2' onClick={() => { setCounter(indexImage => indexImage + 1); setCount(prev => prev + 1); if (count === dataa.length - 5) { updatePage() } }}><FaForward /></button>
